...
news = listdir('/repo/new') # Taking inventory of all new packages, placed in a "/new" directory
for entry in news:
	enpath = '/repo/new/%s' % entry
...
news = listdir('/repo/new') # Taking inventory of all new packages
for file in news:
	newf = path.join('/repo/new', file)
	newfm = path.join('/repo', file)
